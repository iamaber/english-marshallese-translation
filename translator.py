...
from config import OPENAI_API_KEY, MODEL_NAME, TEMPERATURE, VOCAB_FILE_PATH, is_llm_configured
from prompts import (
...
def _load_vocab_dict(vocab_path: Path) -> Dict[str, str]:
    ...
    if not Path(vocab_path).exists():
        warnings.warn(f"Vocab file {vocab_path} not found; fallback translator will be empty.")
        return {}
    ...
    def extract(obj):
        if isinstance(obj, dict):
            # common pair keys
            if "english" in obj and ("marshallese" in obj or "base" in obj or "my" in obj):
...
def _try_use_llm(prompt_system: str, prompt_user: str, text: str) -> str:
    ...
    context = json.dumps(_load_vocab_dict(VOCAB_FILE_PATH), indent=2, ensure_ascii=False)
